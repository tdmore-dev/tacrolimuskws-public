/*
 * License GPLv3, see https://github.com/schaumb/domarrow.js
 * Authors:
 * Initial development by Bela Schaum, 2019
 * bugfixes by Ruben Faelens, 2020-2022
 */
(()=>{
...
// search for "vis-item"
    function findItem(item) {
      ...
      if(item.classList.contains("vis-item")) return(item);
      return(findItem(item.parentNode));
    }
  ...
  function adjustLine(from, to, line, trafo){
    ...
    //console.log("Drawing line from ("+fromB.x+","+fromB.y+") to ("+toB.x+","+toB.y+")");
    ...
    */
    var fT = fromBStartY + fromBHeight * getNumberOrDef(trafo && trafo.fromY, getNumberOrDef(trafo, 0.0));
    var tT = toBStartY + toBHeight * getNumberOrDef(trafo && trafo.toY, getNumberOrDef(trafo, 0.0));
    var fL = fromBStartX + fromBWidth * getNumberOrDef(trafo && trafo.fromX, getNumberOrDef(trafo, 0.0));
    var tL = toBStartX + toBWidth * getNumberOrDef(trafo && trafo.toX, getNumberOrDef(trafo, 0.0));
    ...
    var docB = document.documentElement.getBoundingClientRect();
    top += docB.y;
    left += docB.x;
  ...
  function repaintConnection(connectionElement) {
    ...
    //console.log("Registering connection from "+fromE+" to "+toE);
    
    /// CHANGES BY RFA
    connectedObserver.observe(findItem(fromE), {attributes:true});
    connectedObserver.observe(findItem(toE), {attributes:true});
  ...
  function changedConnectionTag(changes) {
    //console.log("ChangedConnectionTag called with changes: "+changes)
    changes.forEach(e => {
      var conn = e.target;
